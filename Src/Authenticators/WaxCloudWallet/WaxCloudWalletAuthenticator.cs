...
public class WaxCloudWalletConfig
{
    ...
    [Tooltip("URL of the Website implementing WCW, must be either localhost or some remote URL (without SSL! and with Custom script deployed!) ")]
    public string SigningWebsiteUrl;
    [Tooltip("Wether the Website should be hosted on the local device by the local Webserver or if a remote Website (without SSL! and with Custom script deployed!) is used")]
    ...
    [Space(10)]
    [Tooltip("If Host Local Website set to true, the index.html and waxjs.js have to be provided as string. Note that '//'-comments will result in failure, only use '/* ... */'-Comments ")]
